...
public class CollisionCanvas {
	...
	private Vec2f _mousePos = new Vec2f(0, 0);
	...
	public CollisionCanvas() {
		...
		_root.setOnMouseMoved(e -> {
			mouseMoved(new Vec2f(e.getX(), e.getY()));
		});
	...
	public void mousePress(boolean b) {
		...
		if(!_leftMouseDown && b) {
			...
			if(_hovered == null && _week >= 6) {
				_ray = new Ray(_mousePos, Vec2f.ORIGIN);
	...
	public void mouseMoved(Vec2f p) {
		Vec2f src = _mousePos;
		Vec2f distance = p.minus(src);
	...
	public void raycast() {
		if(_ray != null) {
			...
			float xLo = raycastEdge(_ray, true, true);
			float xHi = raycastEdge(_ray, true, false);
			float yLo = raycastEdge(_ray, false, true);
			float yHi = raycastEdge(_ray, false, false);
			// max is intentional- for edge testing, we don't care if we go past the edge, min only matters
			// for shape testing
			float minT = Float.max(Float.max(xLo, xHi), Float.max(yLo, yHi)) + 1;
			float t;
			...
			Vec2f end = _ray.src.plus(_ray.dir.smult(minT));
			_rayG.setEndX(end.x);
			_rayG.setEndY(end.y);
	...
	private float raycastEdge(Ray r, boolean x, boolean lo) {
		float a = x ? r.dir.x : r.dir.y;
		float b;
		if(x) {
