...
public abstract class FXFrontEnd extends CS1971FrontEnd {
  ...
  public FXFrontEnd(String title, Vec2d windowSize, boolean debugMode,
      boolean fullscreen) {
  ...
  protected final void init(Stage stage) {
    ...
    stage.setMinHeight(
        (stage.getHeight() - currentStageSize.y) + MINIMUM_STAGE_SIZE.y);
  ...
  // Helper function to resize the canvas, invoke a resize call, then invoke a
  // draw call
  ...
  // Helper function to ready a new tick, update the title, invoke a tick call,
  // then invoke a draw call
