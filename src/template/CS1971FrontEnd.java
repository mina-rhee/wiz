...
/**
 * Adapted from CS195NFrontEnd, this updated version works with JavaFX (and has
 * only one concrete implementation, {@link FXFrontEnd}).<br>
 * <br>
...
public abstract class CS1971FrontEnd {
  ...
  /**
   * Current window size. Used by direct subclass to set up window based on size
  ...
  /**
   * Fullscreen status. Used by direct subclass to set the window to be
   * fullscreen.
   */
   ...
   * 
   * @param windowSize
   *          a Vec2d assigning the initial window size<br>
   * @param debugMode
   *          a boolean assigning the initial debug mode<br>
   * @param fullscreen
   *          a boolean assigning the initial fullscreen status<br>
   *          <br>
   *          Note: this initial size can be smaller than the minimum window
   *          size. If it is, the new minimum window size will be reduced to
   *          match the size passed in to it. Student projects never need to be
   *          smaller than the default minimum window size of (960, 540).
   */
  public CS1971FrontEnd(Vec2d windowSize, boolean debugMode,
      boolean fullscreen) {
  ...
  /*
   * ____ _____ _ _ ____ _____ _ _ _____ / ___|_ _| | | | _ \| ____| \ | |_ _|
   * \___ \ | | | | | | | | | _| | \| | | | ___) || | | |_| | |_| | |___| |\ | |
   * | |____/ |_| \___/|____/|_____|_| \_| |_|
   * 
   * __ __ _____ _____ _ _ ___ ____ ____ | \/ | ____|_ _| | | |/ _ \| _ \/ ___|
   * | |\/| | _| | | | |_| | | | | | | \___ \ | | | | |___ | | | _ | |_| | |_|
   * |___) | |_| |_|_____| |_| |_| |_|\___/|____/|____/
   */

  /**
   * Called at a regular interval set by {@link #setTickFrequency(long)}. Use to
   * update any state that changes over time.
   * 
  ...
  /**
   * Called when the screen needs to be redrawn. This is at least once per tick,
   * but possibly more frequently (for example, when the window is resizing).
   * <br>
   * <br>
   ...
   * 
   * @param g
   *          a {@link GraphicsContext} object used for drawing.
   */
  protected abstract void onDraw(GraphicsContext g);

  /**
   * @param e
   *          an FX {@link KeyEvent} representing the input event.
   */
  ...
  /**
   * @param e
   *          an FX {@link MouseEvent} representing the input event.
   */
  ...
  /**
   * @param e
   *          an FX {@link ScrollEvent} representing the input event.
   */
  protected abstract void onMouseWheelMoved(ScrollEvent e);

  /**
   * @param newVal
   *          a boolean representing the new focus state
   */
  ...
  /**
   * Called before the window closes by user input. Not guaranteed to be called
   * if the program terminates due to an exception or a System.exit(...) /
   * Platform.close call.
   */
  protected abstract void onShutdown();

  /**
   * Called after the window is initialized by support code, but before it
   * appears. Guaranteed to be called before all events, ticks, resizes, and
   * draw calls.
   */
  protected abstract void onStartup();

  /*
   * ____ _ _ ____ _ ___ ____ | _ \| | | | __ )| | |_ _/ ___| | |_) | | | | _ \|
   * | | | | | __/| |_| | |_) | |___ | | |___ |_| \___/|____/|_____|___\____|
   * 
   * __ __ _____ _____ _ _ ___ ____ ____ | \/ | ____|_ _| | | |/ _ \| _ \/ ___|
   * | |\/| | _| | | | |_| | | | | | | \___ \ | | | | |___ | | | _ | |_| | |_|
   * |___) | |_| |_|_____| |_| |_| |_|\___/|____/|____/
  ...
  /**
   * Enable or disable debug mode. When debug mode is enabled, the current size
   * of the draw area and aspect ratio is displayed in the title of the window
   * along with the current FPS count, and pressing F12 will bring up a dialog
  ...
  /**
   * Controls the frequency of {@link #onTick(long) onTick()} calls. Ticks will
   * occur approximately once every <code>nanoDelay</code> nanoseconds, but
   * <b>no specific accuracy guarantees can be made</b>. Always use the argument
   * of onTick to determine the actual amount of time passed.
   * 
   * @param nanoDelay
   *          the number of nanoseconds between the start of each ticks. For
   *          example, for 50 FPS, this value would be 1000000000/50 (20000000).
   *          Must be > 0.
   */
  ...
  /**
   * Controls the frequency of {@link #onTick(long) onTick()} calls. Ticks will
   * occur approximately <code>fps</code> times each second, but <b>no specific
   * accuracy guarantees can be made.</b> Always use the argument of onTick to
   * determine the actual amount of time passed.
   * 
   * @param fps
   *          the target FPS, must be > 0;
   * 
   */
  ...
  /**
   * Opens up the application for events, no guarantee that this function ever
   * returns
   */
  ...
  /*
   * ____ _ _ ____ ____ ___ ____ _____ / ___|| | | | _ \| _ \ / _ \| _ \_ _|
  ...
  /**
   * Actually run code to go fullscreen or windowed. The <code>fullscreen</code>
   * field will hold the value of the desired new state. When returning from
   * fullscreen back to windowed, it's nice if the subclass remembers the old
   * window size and position, but this behavior is not strictly required.
   */
